...
def build(root: Path, dist: Path, out: Path) -> dict:
    ...
    if policy.get("state") != "PREPARED" or policy.get("publish_enabled") is not False:
        raise EvidenceError(
            "A03 policy must remain PREPARED with publication disabled"
        )

    rows = []
    for path in _artifacts(dist):
        size, value = digest(path)
        rows.append({"name": path.name, "size": size, "sha256": value})
    rows.sort(key=lambda row: row["name"])

    checksum_text = "".join(
        f"{row['sha256']}  {row['name']}\n" for row in rows
    )
    (out / "SHA256SUMS.txt").write_text(checksum_text, encoding="utf-8")
    ...
    bom = {
        ...
        "properties": [
            {
                "name": f"model-router:artifact:{row['name']}:sha256",
                "value": row["sha256"],
            }
            for row in rows
        ],
    }
    (out / "model-router.cdx.json").write_text(
    ...
    receipt = {
        ...
        "repository": "vigilanty0x/model-router",
        "distribution": name,
        "version": version,
        "state": "PREPARED",
        ...
        "artifacts": rows,
        "tests_verified": os.environ.get("MODEL_ROUTER_TESTS_VERIFIED") == "1",
        "counterproof_verified": (
            os.environ.get("MODEL_ROUTER_COUNTERPROOF_VERIFIED") == "1"
        ),
        "signed": False,
        "attested": False,
        "tagged": False,
    ...
    (out / "RELEASE_EVIDENCE.json").write_text(
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("release-evidence-a03"),
    )
    args = parser.parse_args(argv)
    try:
        receipt = build(args.root, args.dist, args.output)
    except (OSError, EvidenceError) as exc:
        raise SystemExit(f"A03 release evidence: {exc}") from exc
    print(
        "A03 release evidence prepared: "
        f"{receipt['distribution']} {receipt['version']} "
